...
*****
## Описание
Проект предназначен для управления мобильным роботом с дифферинциальной кинематикой.
...
>> <em>[Velosity](#Velosity_desc)</em> - Класс для хранения скорости движения робота и угловых скоростей колёс дифферинциального привода.</br>
...
>> <em>[DiffDriveController](#DiffDriveController_desc)</em> - Контроллер управления дифферинциальным приводом. Основан на расчёте кривизны траектории.</br>
...
>> <em>[DiffDriveControllerByPID](#DiffDriveControllerByPID_desc)</em> - Контроллер управления дифферинциальным приводом. Основан на PID регуляторе на вход которого подаётся ошибка направления движения.</br>
...
>> ~~<em>RFIDReader</em> - Класс для работы с RFID метками. Использует сторонюю библиотеку MFRC522.~~ _Необходимо обновление_

### Библиотеки
> <em>lib</em>
>> <em>Encoder</em> - Библиотека для считывания позиции энкодера. Использует сторонюю библиотеку AS5600.</br>
...
- <em>tick</em> - функция для обработки прерывания. Выполняет рассчёт угола поворота колеса за время после предыдущего измерения. Выполняет рассчёт скорости вращения.
...
> :grey_exclamation: Для корректной работы энкодера и рассчёта угла поворота и скорости вращения необходимо постоянно вызывать <em>tick</em>.<br />
...
<a name="Velosity_desc">***Velosity***</a> - Класс для хранения скоростей движения робота (лиейной и угловой) и угловых скоростей колёс дифферинциального привода. Позволяет конвертировать угловую скорость из [rad/s] в частоту оборотов [rmp] и обратно. Позволяет конвертировать угловую скорость колёс [rad/s] в линейную скорость движения [m/s] и обратно.
...
_Основные методы:_
- <em>estCurrentPosition</em> - метод принимает на вход уголы, на которые повернулись колёса, и параметры робота (ширина базы и радиус колеса) и рассчитывает текущее местоположение и ориентацию мобильного робота.

> :grey_exclamation: На данный момент колёсная одометрия рассчитывается в рамках класса <em>[Position](#Position_desc)</em>. Данные о углах поворта колёс для оценки местоположения берутся из класса <em>[MotorBlock](#MotorBlock_desc)</em>.

<a name="MotionController_desc">***MotionController***</a> - Контроллер управления движением. Используется для оценки положения мобильного робота относительно реального местоположения и целевой координаты и для рассчёта желаемых скоростей движения робота для достижения заданной координаты. Для расчёта желаемых скоростей использует контроллер управления дифферинциальным приводом <em>[DiffDriveController](#DiffDriveController_desc)</em>.</br>
Возможности:
...
<a name="DiffDriveController_desc">***DiffDriveController***</a> - Контроллер управления дифферинциальным приводом. Основан на расчёте кривизны траектории.</br>
Возможности:
...
Разработанный алгоритм рассчёта желаемых скоростей основан на следующей статье:</br>
...
<a name="DiffDriveControllerByPID_desc">***DiffDriveControllerByPID***</a> - Контроллер управления дифферинциальным приводом. Основан на PID регуляторе, на вход которого подаётся ошибка направления движения.</br>
Возможности:
- Расчёт желаемых скоростей робота (линейная и угловая).
- Расчёт желаемых скоростей колёс дифферинциального привода.
